...
const LoadingSpinner = React.forwardRef<HTMLDivElement, LoadingSpinnerProps>(
  (props, ref) => {
    ...
    const borderStyle = cn(
      `${spinnerStyles} border-${
        borderColor || defaultSpinnerColors.border
      } border-t-${borderTopColor || defaultSpinnerColors.borderTop}`
    );

    const sizeStyle = size ? `h-${size} w-${size}` : defaultSpinnerColors.size;

    return (
      <div ref={ref} className={cn(sizeStyle, className)} {...rest}>
